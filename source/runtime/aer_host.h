...
/* True if name was registered as a module via aer_register_function; consulted by chunk_add_import (vm.c)
   so `import <name>` accepts it. */
bool aer_host_is_module(const char* name, unsigned int len);

/* Invokes module.fn_name with arg_count arguments already on vm's stack -- pops them, calls the host
   function, pushes its result; returns false (stack untouched) if module/fn_name don't resolve. */
