...
struct AerArray {
    ...
    /* Bumped on every mutation that can change which shapes occupy items[] -- index-assignment
       ...
       unaffected by order). Lets h_call's SPEC_KIND_ARRAY_OF_STRUCTS per-call-site cache (call.c)
...
struct AerFunction {
    ...
    unsigned short frame_bounds; /* FRAME_BOUNDS -- see ChunkFunction's own field, vm.h */
