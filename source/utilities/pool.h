...
/* A slab (bump/arena) allocator for fixed-size objects, extended for generational mark-and-sweep GC (see README's Memory and Security section). Allocation hands out a free-list cell if one exists, else the next cell in the current slab, growing by one fresh slab (one xmalloc, not one per object) when exhausted.
   ...
   Each cell has one state byte in `cell_state` (parallel to `slabs`): bit 0
   is the mark bit (this collection cycle only); bit 1 is the generation bit
   (0 = young, 1 = old, set once a cell survives a collection); bit 2 marks a
   cell on the free-list — this is what stops pool_sweep from re-discovering
   an already-free cell (nothing marks it) and re-pushing it, corrupting the
   list; bit 3 marks a cell already in the GC's remembered set (gc_remember,
   vm.c), checked before scanning the remembered set so re-remembering costs
   one lookup instead of a full scan, and is never cleared once set (matches
   the remembered set's own add-only design — see vm.c). Byte-per-cell, not
   bit-packed, per this project's readability-over-micro-opt precedent.
...
typedef struct {
    ...
    void*           free_list;       /* singly-linked through freed cells' first sizeof(void*) bytes */
} Pool;
...
/* Pushes cell back onto the free-list — called only from pool_sweep. */
