...
static bool string_upper(VM* vm) {
    AerVal a = vm_stack_pop(vm);
    if (aer_type(a) != TYPE_STRING) {
...
static bool string_lower(VM* vm) {
    AerVal a = vm_stack_pop(vm);
    if (aer_type(a) != TYPE_STRING) {
...
static bool string_contains(VM* vm) {
...
static bool string_index_of(VM* vm) {
    AerVal needle = vm_stack_pop(vm);
    AerVal hay = vm_stack_pop(vm);
    if (aer_type(hay) != TYPE_STRING || aer_type(needle) != TYPE_STRING) {
...
static bool string_split(VM* vm) {
...
static bool string_starts_with(VM* vm) {
...
static bool string_ends_with(VM* vm) {
    ...
    bool matches =
        fs->length <= ss->length && memcmp(ss->data + (ss->length - fs->length), fs->data, fs->length) == 0;
    vm_stack_push(vm, aer_bool(matches));
    return true;
}
static bool string_repeat(VM* vm) {
...
static bool string_replace(VM* vm) {
