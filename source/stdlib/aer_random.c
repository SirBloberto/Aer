...
bool aer_random_call(VM* vm, int fn_id, int arg_count) {
    if (fn_id == FN_RANDOM_RANDOM && arg_count == 0) {
        ...
        vm_stack_push(vm, aer_real((double)(rng_next() >> 11) * (1.0 / 9007199254740992.0)));
        return true;
    }
    if (fn_id == FN_RANDOM_RANDINT && arg_count == 2) {
        AerVal hi = vm_stack_pop(vm);
        AerVal lo = vm_stack_pop(vm);
        if (aer_type(lo) != TYPE_INTEGER || aer_type(hi) != TYPE_INTEGER) {
            error("randint() requires two integers");
            vm_stack_push(vm, aer_null());
            return true;
        }
        int64_t lo_n = aer_as_int(lo), hi_n = aer_as_int(hi);
        if (hi_n < lo_n) {
            error("randint() requires min <= max");
            vm_stack_push(vm, aer_null());
            return true;
        }
    ...
    if (fn_id == FN_RANDOM_CHOICE && arg_count == 1) {
        ...
        if (aer_type(arr) != TYPE_ARRAY || aer_as_array(arr)->shape) {
            error("choice() requires an array");
            vm_stack_push(vm, aer_null());
            return true;
        }
        AerArray* a = aer_as_array(arr);
        if (a->count == 0) {
            error("choice() requires a non-empty array");
            vm_stack_push(vm, aer_null());
            return true;
        }
        vm_stack_push(vm, a->items[rng_below(a->count)]);
        return true;
    }
    if (fn_id == FN_RANDOM_SHUFFLE && arg_count == 1) {
        AerVal arr = vm_stack_pop(vm);
        if (aer_type(arr) != TYPE_ARRAY || aer_as_array(arr)->shape) {
            error("shuffle() requires an array");
            vm_stack_push(vm, aer_null());
            return true;
        }
        AerArray* a = aer_as_array(arr);
