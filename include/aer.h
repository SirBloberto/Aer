...
/* Caps total live cells across all six pools — a -Xmx-style ceiling; 0 (the default) means unlimited. If a script is still over the ceiling immediately after a forced extra collection, it aborts with a normal, recoverable runtime error (aer_last_error()), the same non-fatal path every other runtime fault takes, not a process exit. */
