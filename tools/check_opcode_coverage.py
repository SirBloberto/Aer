...
def declared_opcodes():
    """Every opcode row in source/bytecode/opcodes.def."""
    src = open(os.path.join(ROOT, "source", "bytecode", "opcodes.def"), encoding="utf-8").read()
    names = re.findall(r"^OPCODE\(([A-Z0-9_]+),", src, re.M)
    if not names:
        sys.exit("could not find any OPCODE rows in source/bytecode/opcodes.def")
